...
extension HCache {
    ...
    final class Manager: Sendable {
        ...
        nonisolated let diskQueue = DispatchQueue(label: "harbor.cache.disk", qos: .utility)
        ...
        func removeCachedData(for key: String) {
            ...
            diskQueue.async {
                let (dataURL, metaURL) = self.fileURLs(for: key)
        ...
        nonisolated private func fileURLs(for key: String) -> (URL, URL) {
        ...
        private func removeExpiredFiles() {
            ...
            for metaURL in metaFiles {
                ...
                // If expired, delete both meta and data file
                // Using a default maxAge of distantFuture since we only want to clear if the specific entry has an expirationTime set and passed
                let entry = Entry(data: Data(), timestamp: metadata.timestamp, expirationTime: metadata.expirationTime)
