...
extension HJRPCRequestManager {
    static func batch(requests: [any HJRPCRequestProtocol]) async -> [HJRPCBatchResponse] {
        ...
        let harborRequest = HJRPCBatchWrapper(debugType: .none, rawBody: rawBody, requestIDs: requestIDs, url: config.url, needsAuth: needsAuth, retryPolicy: nil, pathParameters: nil, headerParameters: nil)
        let response: HResponseWithResult = await harborRequest.request()
